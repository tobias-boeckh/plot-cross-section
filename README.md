...
The `plot-cross-section.ipynb` is the main plotting script. The `src/utils` directory contains utility scripts to define plot style, calculate the simulated cross section and plotting scripts, the `data` directory contains new measured cross section, the `figures` directory the final plots.
